...
LenaJS.pixelate = function(pixels,blockSize) {
  var blockColor = [];
  //We have the blocks
  for (var x = 0; x < canvasWidth; x++) {
    for (var y = 0; y < canvasHeight; y++) {
      colX = floor(x/blockSize);
      colY = floor(y/blockSize);

      if(!blockColor[colX]) blockColor[colX] = [];
      if(!blockColor[colX][colY]) blockColor[colX][colY] = {'R':0,'G':0,'B':0};

      blockColor[colX][colY].R += pixels.data[(x+y*canvasWidth)*4];
      blockColor[colX][colY].G += pixels.data[(x+y*canvasWidth)*4+1];
      blockColor[colX][colY].B += pixels.data[(x+y*canvasWidth)*4+2];
    }
  }
  //We linearize the color
  for(var colX=0;colX<floor(width/resolution);colX++) {
    for(var colY=0;colY<floor(height/resolution);colY++) {
      blockColor[colX][colY].R  = Math.floor(blockColor[colX][colY].R  / blockSize / blockSize);
      blockColor[colX][colY].G  = Math.floor(blockColor[colX][colY].G  / blockSize / blockSize);
      blockColor[colX][colY].B  = Math.floor(blockColor[colX][colY].B  / blockSize / blockSize);
    }
  }
  //We redispatch the color
  for (var x = 0; x < canvasWidth; x++) {
    for (var y = 0; y < canvasHeight; y++) {
      colX = floor(x/blockSize);
      colY = floor(y/blockSize);
      ...
      pixels.data[(x+y*canvasWidth)*4] = blockColor[colX][colY].R;
      pixels.data[(x+y*canvasWidth)*4+1] = blockColor[colX][colY].G;
      pixels.data[(x+y*canvasWidth)*4+2] = blockColor[colX][colY].B;
    }
  }
  return pixels;
}

LenaJS.boost = function(pixels,threshold,boost) {
  var blockColor = [];
  //We have the blocks
  for (var x = 0; x < canvasWidth; x++) {
    for (var y = 0; y < canvasHeight; y++) {
      for(var i = 0;i<3;i++) {
        pixels.data[(x+y*canvasWidth)*4+i] = (pixels.data[(x+y*canvasWidth)*4+i] < threshold && pixels.data[(x+y*canvasWidth)*4+i] > boost)?pixels.data[(x+y*canvasWidth)*4+i] - boost : pixels.data[(x+y*canvasWidth)*4+i];
      }
    }
  }
  return pixels;
